...
from . import HtmlParserProtocol
from ..models import Movie, MovieReviewStats
...
class RottenTomatoesMovieReviewScraper:
    ...
    def __parse_movie_scores(self, parser: HTMLParser, url: str) -> MovieReviewStats:
        """Parses the rottentomatoes scores from the movie details page"""
        stats = MovieReviewStats(site="rottentomatoes")
        ...
        stats.audience.score = audience_score
        stats.audience.review_count = audience_count

        critic_score, critic_count = self.__parse_critic_scoreboard(score_board_node)
        stats.critic.score = critic_score
        stats.critic.review_count = critic_count
    ...
    async def get_movie_urls(
        self, client: AsyncClient, movie_titles: Iterable[tuple[int, str]]
    ) -> list[tuple[int, str]]:
        ...
        for movie_id, title in movie_titles:
            search_url = SEARCH_URL.format(search=urllib.parse.quote(title))
            parser = await self.scraper.get_html_parser(client, search_url)
            if parser is not None:
                movie_url = self.__get_movie_url(parser, title)
                if movie_url is not None:
                    all_movie_urls.append((movie_id, movie_url))

        return all_movie_urls

    async def run(self, movies: Iterable[Movie]) -> list[MovieReviewStats]:
        stat_results: list[MovieReviewStats] = []
        async with httpx.AsyncClient() as client:
            movie_urls = await self.get_movie_urls(client, [(m.id, m.title) for m in movies])

            for movie_id, movie_url in movie_urls:
                parser = await self.scraper.get_html_parser(client, movie_url)
                if parser is not None:
                    stats = self.__parse_movie_scores(parser, movie_url)
                    stats.movie_id = movie_id
