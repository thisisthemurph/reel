...
class MoviesRepo:
    ...
    def update(self, movie_id: int, movie: Movie):
        self.supabase.table(Table.Movies).update(movie.supabase_dict).eq("id", movie_id).execute()
    ...
    def get_by_title(self, title: str) -> Movie | None:
        movie = self.supabase.table(Table.Movies).select("*").eq("title", title).limit(1).execute()
